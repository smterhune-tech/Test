...
const SPARK_USER_AGENT = process.env.SPARK_USER_AGENT || "RelocationExpertArizona/1.0";
...
// for a Phoenix-metro deployment this MLS is Arizona Regional MLS (ARMLS),
// which is itself built on the Flexmls/Spark platform.
const COMMUNITY_FIELD_MAP = {
  "old-town-scottsdale": { city: "Scottsdale", subdivision: "Old Town" },
  "arcadia": { city: "Phoenix", subdivision: "Arcadia" },
  "gilbert": { city: "Gilbert", subdivision: null },
  "paradise-valley": { city: "Paradise Valley", subdivision: null },
  "cave-creek": { city: "Cave Creek", subdivision: null }
};
...
function buildFilter({ community, city, query, minBeds, pool }) {
  ...
  const minBedsNum = Number(minBeds);
  if (Number.isFinite(minBedsNum) && minBedsNum > 0) {
    clauses.push(`BedsTotal Ge ${minBedsNum}`);
  }
...
async function searchListings({ community, city, query, minBeds, pool, newOnly, limit } = {}) {
  if (!isLive()) {
    ...
    const minBedsNum = Number(minBeds);
    if (Number.isFinite(minBedsNum) && minBedsNum > 0) items = items.filter((l) => l.beds >= minBedsNum);
    const limitNum = Number(limit);
    if (Number.isFinite(limitNum) && limitNum > 0) items = items.slice(0, limitNum);
    return { source: "mock", listings: items };
  }

  const filter = buildFilter({ community, city, query, minBeds, pool });
  const limitNumLive = Number(limit);
  const data = await sparkFetch("/listings", {
    _filter: filter,
    _limit: Number.isFinite(limitNumLive) && limitNumLive > 0 ? limitNumLive : 24,
